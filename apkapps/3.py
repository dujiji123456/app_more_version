...
class APKPureScraper:
                ...
                if version_list[-1].xpath('@class')[0] == 'more-version':  # 判断是否需要获取更多版本链接
                    ...
                    for i in version_list:  # 遍历版本列表
                        ...
                        if root.xpath('//div[@class="module change-log"]'):  # 判断是否存在更新日志
                            ...
                            data_item = {'apk_name': q, 'apk_version': apk_version, 'apk_download_url': apk_url, 'change_log': change_log}  # 创建数据字典
                        else:
                            data_item = {'apk_name': q, 'apk_version': apk_version, 'apk_download_url': apk_url, 'change_log': ''}  # 创建数据字典
                        self.data.append(data_item)  # 将数据字典添加到数据列表中
    ...
    def main(self, q):
        ...
        if data_list == '获取失败':
            ...
            # 延时到第二天早上再执行上传任务
            # tomorrow = time.time() + 24 * 60 * 60  # 获取明天的时间戳
            # midnight = time.mktime(
            #     time.strptime(time.strftime("%Y-%m-%d") + " 00:00:00", "%Y-%m-%d %H:%M:%S"))  # 获取午夜时间戳
            # delay_seconds = midnight - time.time()  # 计算距离午夜的延时秒数
            #
            # if delay_seconds > 0:
            #     time.sleep(delay_seconds)  # 延时到午夜
            # else:
            #     tomorrow += 24 * 60 * 60  # 如果已过了午夜，则将上传任务延到下一天早上
            #     time.sleep(tomorrow - time.time())  # 延时到第二天早上
...
if __name__ == '__main__':
    ...
    state = apk_scraper.main('com.papegames.lysk.en') # 调用main函数爬取数据
    print(state)  # 打印结果