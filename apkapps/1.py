...
class APKPureScraper:
    ...
    def spider(self, q, lang):
        try:
            url = f"https://apkpure.net/{lang}search?q={q}"  # 搜索页链接
                ...
                if version_list[-1].xpath('@class')[0] == 'more-version':  # 判断是否需要获取更多版本链接
                    ...
                    for li in li_list[:-1]:  # 遍历版本列表
                        ...
                        if root.xpath('//div[@class="module change-log"]'):  # 判断是否存在更新日志
                            ...
                            data_item = {'apk_name': q, 'lang':lang,'apk_version': apk_version, 'apk_download_url': apk_download_url, 'change_log': change_log}  # 创建数据字典
                        else:
                            data_item = {'apk_name': q, 'lang':lang,'apk_version': apk_version, 'apk_download_url': apk_download_url, 'change_log': ''}  # 创建数据字典
                    ...
                    for i in version_list:  # 遍历版本列表
                        ...
                        if root.xpath('//div[@class="module change-log"]'):  # 判断是否存在更新日志
                            ...
                            data_item = {'apk_name': q, 'lang':lang,'apk_version': apk_version, 'apk_download_url': apk_url, 'change_log': change_log}  # 创建数据字典
                        else:
                            data_item = {'apk_name': q, 'lang':lang,'apk_version': apk_version, 'apk_download_url': apk_url, 'change_log': ''}  # 创建数据字典
                        self.data.append(data_item)  # 将数据字典添加到数据列表中
        ...
        for item in data_list:
            try:
                ...
                self.cursor.execute(sql, (
                    item['apk_version'], item['apk_name'], item['apk_download_url'], item['change_log']))  # 执行插入语句
    ...
    def main(self, q,lang):
        data_list = self.spider(q,lang)  # 调用spider函数获取数据列表
        if data_list == '获取失败':
...
if __name__ == '__main__':
    ...
    state = apk_scraper.main('jp.konami.duellinks','vn/') # 调用main函数爬取数据
